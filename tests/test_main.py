...
def test_extract_morphemes_success(monkeypatch, mock_json_files, mock_dataframe):
    ...
    with (
        patch("main.find_json_files", return_value=mock_json_files)
        as mock_find_json_files,
        patch("main.get_morphemes_from_songs", return_value=mock_dataframe)
        as mock_get_morphemes,
        patch("main.save_to_db") as mock_save_to_db,
    ):
        response = client.post(
            "/extract-morphemes/", json={"json_dir": "test_dir"}
        )
        assert response.status_code == 200
        assert (
            response.json()["message"]
            == "Morphemes extracted and saved to database."
        )
...
def test_extract_morphemes_no_json_files(monkeypatch):
    ...
    with patch("main.find_json_files", return_value=[]) as mock_find_json_files:
        response = client.post(
            "/extract-morphemes/", json={"json_dir": "empty_dir"}
        )
...
def test_extract_morphemes_empty_dataframe(monkeypatch, mock_json_files):
    ...
    with (
        patch("main.find_json_files", return_value=mock_json_files)
        as mock_find_json_files,
        patch("main.get_morphemes_from_songs", return_value=pd.DataFrame())
        as mock_get_morphemes,
    ):
        response = client.post(
            "/extract-morphemes/", json={"json_dir": "test_dir"}
...
def test_visualize_success(monkeypatch, mock_dataframe):
    ...
    monkeypatch.setenv("DB_NAME", "testdb")
    with (
        patch("main.load_morpheme_table", return_value=mock_dataframe)
        ...
        patch("main.setup_visualization", return_value={"title": 10})
