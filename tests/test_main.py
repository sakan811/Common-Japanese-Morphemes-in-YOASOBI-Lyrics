...
def test_main_no_json_files():
    with patch('main.find_json_files') as mock_find_json_files, \
        ...
        mock_logger_warning.assert_called_once_with("No JSON files found in the specified directory.")
...
def test_main_empty_dataframe(mock_json_files):
    with patch('main.find_json_files') as mock_find_json_files, \
        ...
        mock_get_morphemes.assert_called_once_with(mock_json_files)
        mock_save_to_sqlite.assert_not_called()
        mock_logger_warning.assert_called_once_with("No morphemes found in the JSON files.")