...
def mock_dependencies():
    with patch('morphemes_extractor.data_extractor.load_json') as mock_load_json, \
            ...
            patch('morphemes_extractor.data_extractor.transform_data_to_df') as mock_transform_data_to_df:
        yield (mock_load_json, mock_get_song_list, mock_extract_data, mock_transform_data_to_df)


def test_get_morphemes_from_songs_valid_data(mock_dependencies):
    mock_load_json, mock_get_song_list, mock_extract_data, mock_transform_data_to_df = mock_dependencies

    mock_load_json.return_value = {'songs': [{'title': 'Song 1'}]}
    mock_get_song_list.return_value = [{'title': 'Song 1'}]
    mock_extract_data.return_value = (['word1'], ['romanized1'], ['pos1'], 'Song 1', 'Song One')
